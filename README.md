...
3. 글자 크기 조절 버튼을 `-`, `기본`, `+` 3개로 단순화
4. `-`, `+` 표시는 이전보다 더 크게 보이도록 조정
5. `-` 버튼은 누를 때마다 **`-0.2 → -0.4 → 기본(0)`** 순서로 순환
6. `+` 버튼은 누를 때마다 **`+0.2 → +0.4 → 기본(0)`** 순서로 순환
7. 전체 보기에서 선택한 핵심 문장은 **글자색만 보라색**으로 표시
8. 하이라이터 버튼은 **노랑 → 분홍 → 해제** 순서로 순환
9. 저작권 문구는 **좌측 하단**에 더 큰 폰트로 표시
